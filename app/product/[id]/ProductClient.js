...
import { ArrowRight, ShieldCheck, Truck, Gem, Sparkles } from 'lucide-react';
...
export default function ProductClient({ id }) {
    ...
    useEffect(() => {
        // Fetch products from API since we are on client
        fetch('/api/products')
            .then(res => res.json())
            .then(products => {
    ...
    if (!product) {
        return (
            <div className="min-h-screen bg-black flex flex-col items-center justify-center p-6">
                <h1 className="text-2xl font-bold mb-4 text-white">{t('product.notFound')}</h1>
                <Link href="/shop" className="btn btn-primary">{t('product.backToShop')}</Link>
            </div>
        );
    }

    const localizedName = getLocalized(product, 'name');
    const localizedDesc = getLocalized(product, 'description');
                    ...
                    <div className="flex flex-col animate-in fade-in slide-in-from-right duration-700">
                        <div className="mb-8">
                            <div className="flex items-center gap-2 text-primary text-xs uppercase tracking-[0.4em] font-bold mb-4 opacity-70">
                                <Sparkles size={14} />
                                <span>{t('product.luxuryCrafting')}</span>
                            </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-10">
                            <FeatureBadge
                                ...
                                desc={t('product.authenticityDesc')}
                            />
                            <FeatureBadge
                                icon={<Truck size={20} />}
                                title={t('product.delivery')}
                                desc={t('product.deliveryDesc')}
                            />
                        </div>
