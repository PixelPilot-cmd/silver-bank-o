...
export default function ShopClient({ initialProducts }) {
    ...
    if (searchQuery) {
        products = products.filter(p =>
            ...
            (p.name_he && p.name_he.toLowerCase().includes(searchQuery)) ||
            (p.description && p.description.toLowerCase().includes(searchQuery)) ||
            (p.description_en && p.description_en.toLowerCase().includes(searchQuery)) ||
            (p.description_he && p.description_he.toLowerCase().includes(searchQuery))
        );
    }
        ...
        <main className="min-h-screen pb-20">
            ...
            <div className="container mx-auto px-4 py-12">
                <div className="text-center mb-12 animate-in fade-in slide-in-from-bottom duration-700">
                    <h1 className="text-4xl md:text-6xl font-serif font-bold mb-4">{t('shop')}</h1>
                    ...
                    <div className="flex flex-wrap items-center gap-4 mb-10">
                        ...
                        {searchQuery && (
                            <div className="flex items-center gap-3 bg-primary/10 px-5 py-2.5 rounded-full border border-primary/20 text-sm animate-in fade-in slide-in-from-right duration-700">
                                ...
                                <span className="text-white font-light tracking-wide">{t('searchResults')}: <span className="font-bold">"{searchQuery}"</span></span>
                            ...
                            {products.map((product) => (
                                <Link href={`/product/${product.id}`} key={product.id} className="group relative bg-transparent border border-white/10 rounded-xl overflow-hidden hover:border-white/30 transition-all duration-500 flex flex-col">
                                    ...
                                    <div className="p-5 text-center flex-1 flex flex-col justify-between bg-gradient-to-b from-[#0a0a0a] to-black">
                                        <div>
                                            <h3 className="text-lg font-bold text-white mb-2 font-serif tracking-wide">{getLocalized(product, 'name')}</h3>
                                            <p className="text-sm text-gray-500 font-light tracking-wider">{categories.find(c => c.id === product.category)?.name || t('luxuryItem')}</p>
                                        </div>
